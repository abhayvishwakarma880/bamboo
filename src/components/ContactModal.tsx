...
import { type EventType, getPostVerificationRoute } from '../lib/eventRoute';
...
const ContactModal: React.FC<ContactModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasPermanentAccess) {
        ...
        if (resolvedRequestId) {
          fallbackQueryParams.set('requestId', resolvedRequestId);
        }

        const fallbackRoute = fallbackQueryParams.toString()
          ? `${getPostVerificationRoute(resolvedPortfolioId, eventType)}?${fallbackQueryParams.toString()}`
          : getPostVerificationRoute(resolvedPortfolioId, eventType);
  ...
  const handlePinVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`${getPostVerificationRoute(resolvedPortfolioId, eventType)}?${queryParams.toString()}`);
    } catch (error) {
      setIsVerifying(false);
