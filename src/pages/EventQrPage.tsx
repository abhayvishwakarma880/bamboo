...
import { getDefaultPortfolioIdForEventType, rememberPortfolioIdForEventType } from '../lib/eventRoute';
...
const EventQrPage: React.FC = () => {
  ...
  const fallbackPortfolioId = content
    ? getDefaultPortfolioIdForEventType(eventType as EventType)
    : undefined;
  ...
  useEffect(() => {
    ...
    rememberPortfolioIdForEventType(eventType as EventType, resolvedPortfolioId);
  }, [content, eventType, resolvedPortfolioId]);
  ...
  const qrQueryParams = new URLSearchParams({ openModal: '1' });

  if (resolvedPortfolioId) {
    qrQueryParams.set('portfolioId', String(resolvedPortfolioId));
  }

  const qrUrl = `${getAppBaseUrl()}/events/${eventType}?${qrQueryParams.toString()}`;
